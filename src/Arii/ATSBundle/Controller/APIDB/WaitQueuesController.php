...
class WaitQueuesController extends Controller
{
    public function listAction($repoId='ats_db')
    {
        ...
        $Queues = $state->WaitQueues($em);

        switch ($Filters['outputFormat']) {
            case 'dhtmlxGrid':
                $dhtmlx = $this->container->get('arii_core.render'); 
                return $dhtmlx->grid($Queues,'parentName,machName,machStatus,Status,description,type,maxLoad,factor,opsys,opSys,characterCode,port,prepjobid,provision');        
                break;
            case 'xml':
