...
class Orders2Controller extends Controller
{
    ...
    public function pieAction($db,$history_max=0,$nested=false,$onlyWarning=true)
    {
        $request = Request::createFromGlobals();
        ...
        $history->setDB($db);        
        $Orders = $history->Orders($history_max,$nested,$onlyWarning);
    ...
    public function gridAction($db,$history_max=0,$nested=false,$onlyWarning=false,$sort='last')
    {
        $Filters = $this->container->get('arii.filter')->getRequestFilter();

        $em = $this->getDoctrine()->getManager($db);        
        $history = $this->container->get('arii_jid.history2');        
        $Orders = $history->Orders($em,$Filters['start'],$Filters['end'],$Filters['maxResults'],$Filters['sort'],$Filters['onlyWarning']);
...
//                                $cell .= '<cell><![CDATA['.$job_name.' '.$state_text.']]></cell>';
    ...
    public function atomAction($history_max=0,$nested=false,$onlyWarning=true,$sort='last')
    {
        $request = Request::createFromGlobals();
        if ($request->get('history')>0) {
            $history_max = $request->get('history');
        ...
        $history = $this->container->get('arii_jid.history');     
        $Orders = $history->Orders($history_max,$nested,$onlyWarning,$sort);
