...
class RUNRepository extends EntityRepository
{
    public function findRuns($Filter=[]) { 
        ...
        ->select( 'e.id,e.spooler_name,e.job_name,e.job_trigger,(e.job) as job_id,e.status,e.machine,e.start_time,e.end_time,e.run,e.try,e.exit_code,e.alarm_time,e.alarm,e.ack_time,e.ack,e.message')
        ->orderBy('e.start_time')
        ->setMaxResults(1000);
        # Filtrage
        if (isset($Filter['spooler_name']))
            $q->andWhere('e.spooler_name = :spooler_name')
                ->setParameter('spooler_name',$Filter['spooler_name']);           
        if (isset($Filter['job_name']))
            $q->andWhere('e.job_name like :job_name')
                ->setParameter('job_name',$Filter['job_name']);           
        if (isset($Filter['start_time']))
            $q->andWhere('e.start_time >= :start_time')
                ->setParameter('start_time',$Filter['start_time']);           
        if (isset($Filter['end_time']))
            $q->andWhere('e.end_time <= :end_time')
                ->setParameter('end_time',$Filter['end_time']);           
        if (isset($Filter['exit_code_min']))
            $q->andWhere('e.exit_code >= :exit_code_min')
                ->setParameter('exit_code_min',$Filter['exit_code_min']);           
        if (isset($Filter['exit_code_max']))
            $q->andWhere('e.exit_code <= :exit_code_max')
                ->setParameter('exit_code_max',$Filter['exit_code_max']);           
        if (isset($Filter['status']))
            $q->andWhere('e.status = :status')
                ->setParameter('status',$Filter['status']);           
        if (isset($Filter['duration_min']))
            $q->andWhere('DATE_DIFF(e.start_time,e.end_time) >= :duration_min')
                ->setParameter('duration_min',120);           
/*
